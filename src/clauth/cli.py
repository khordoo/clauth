...
def prompt_for_region_if_needed(config, cli_overrides):
    ...
    if not cli_overrides.get("region"):
        ...
        region_options = [
            ...
            "ap-southeast-2", "ap-northeast-1", "ca-central-1", custom_region_option
        ]
        ...
        selected_option = inquirer.select(
            ...
            default=config.aws.region if config.aws.region in region_options else "us-east-1",
            pointer="> ",
            amark="✔",
        ).execute()
...
def delete_aws_credentials_profile(profile_name: str) -> bool:
    ...
    try:
        ...
        if not aws_credentials_file.exists():
            console.print("[yellow]No AWS credentials file found to delete profile from.[/yellow]")
            return True
        ...
        console.print(f"[red]ERROR: Unexpected error deleting AWS credentials profile: {e}[/red]")
        return False
...
def delete_aws_profile(profile_name: str) -> bool:
    ...
    try:
        ...
        if not aws_config_file.exists():
            console.print("[yellow]No AWS config file found to delete profile from.[/yellow]")
            return True
